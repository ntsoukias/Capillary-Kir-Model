...
%% Simulation time
Tmax = 15;                % [s] Total time of the simulation
...
%% solution mode
% default is barrel (as shown in manuscript)
% other option is "generation" to use capillary generations from arteriole

Options.SensitizeMode = 'barrel';

% Options.SensitizeMode = 'generation'; 
% ngen = 9;   % desired number of generations (default: 9)
...
seedForBarrel = [300 1200 -950];
xbarrel=120; ybarrel=120;  zbarrel= 1000;

%% Parameters

Rgj_cap = 400e-3; % [Gohm] cEC Rgj
...
G_Kir_cap   =   6/sqrt(3)*G_bg_cap ;
G_Kir_PA    =   3* G_bg_PA;
G_Kir_pial  =   3 * G_bg_pial;

ratioBarrel = 14; % Gkir/Gbg ratio to be used for a sensitized barrel around the arteriole
...
%% make capillary generations from arterioles
if strcmp(Options.SensitizeMode,'generation')
    indType1 = find(G.Edges.Type == 1);
    
    edgelist = G.Edges{indType1,1};
    span = 2*length(edgelist);
    artnodes = unique(reshape(edgelist,[span 1]));
    deglist = degree(G, artnodes);
    artInd = find(deglist > 2);
    
    disp('Labeling Cap generations....')
    G = labelGenerations(G, ngen, artnodes(artInd));
    
    % view labeled generations
    subG_with_gen = subfromedge(G, [indType1; find(G.Edges.Gen > 0)]);
    figure, plotgenedges(subG_with_gen);
end
...
desiredLArt = 33;   %um desired length of arterioles
...
if strcmp(Options.SensitizeMode,'barrel')
    barrelInd = getnodefromsourceXYZ(G, seedForBarrel, xbarrel, ybarrel, zbarrel);
else
    barrelInd = [];
end
...
if strcmp(Options.SensitizeMode,'generation')
    GKirbarCapGen = linspace(0.75, 0.4, ngen);
    for i = 1:ngen
        GenKir = GKirbarCapGen(i);
        gen = find(G.Nodes.Gen == i);
        G_kirbar(gen) = GenKir;
    end
end

G_kirbar(artInd) = G_Kir_PA;   % [nS/mM^0.5] inward rectifier constant
...
[~,venInd] = subfromedge(G,find(G.Edges.Type == 2));
figure
plot(tplot, Vm_post(:,capInd(1:20:end)),'color',[0.7 0.7 0.7],'linewidth',2), hold on
...
function cmap = grey2red(n)


R = [1 0 0];   % color first row - red
G = [1 1 0];   % color middle row - white
...
cmap = grey2red(max(G.Nodes.Gen));
var = linspace(0.1, 1, max(G.Nodes.Gen));
for i = 1:max(G.Nodes.Gen)
    genInd = find(G.Edges.Gen == i);
    color = cmap(i, :);
    highlight(H,'Edges',genInd,'EdgeColor',color,'LineWidth',3)
end
